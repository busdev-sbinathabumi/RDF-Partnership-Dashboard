...
// FIX: Removed 'Font' from import as it is not an exported member of 'chart.js'.
...
// --- NEW --- Synthesized Risk, Cost, and Contract Data
...
// --- QUALITY ANALYSIS PAGE COMPONENT ---
...
// --- NEW --- COST BENEFIT ANALYSIS PAGE ---
    ...
    useEffect(() => {
        const highRiskData = data
            ...
            .map(item => ({
                ...
                label: item.pemda,
                r: 10 // Larger radius for high-risk
            }));
...
const EnvironmentalImpactPage: React.FC<{ setView: (view: string) => void; data: DashboardItem[] }> = ({ setView, data }) => {
    ...
    const CO2_REDUCTION_PER_TON_RDF = 1.8; // Tons of CO2 saved per Ton of RDF used
    const MSW_TO_RDF_RATIO = 1.5; // Tons of MSW needed to produce 1 Ton of RDF
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!mapRef.current) {
                ...
                if (item.latitude && item.longitude) {
                    ...
                    marker.on('popupopen', () => {
                        ...
                        if (item.closestFactory) {
                            // FIX: Casting options to 'any' to resolve type mismatches for properties like 'createMarker'.
            ...
            // This ensures the map resizes correctly when switching back to the dashboard view
            setTimeout(() => {
